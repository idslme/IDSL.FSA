FSA_workflow <- function(spreadsheet) {
  ...
  if (length(PARAM_FSA) > 0) {
    ...
    .GlobalEnv$.logFSA <- paste0(output_path, "/logFSA_performance.txt")
    ...
    FSA_logRecorder("", printMessage = FALSE)
    FSA_logRecorder(paste0(PARAM_FSA[, 1], "\t", PARAM_FSA[, 2]),  printMessage = FALSE)
    FSA_logRecorder(paste0(rep("", 100), collapse = "-"))
    ##
    ############################################################################
    ##
    ...
    if (FSA0001 == "yes") {
      ...
      ##
      .GlobalEnv$.logFSA <- paste0(output_path, "/logFSA_msp_annotation.txt")
      FSA_logRecorder(paste0(rep("", 100), collapse = "="))
      ##
    ...
    FSA_logRecorder(paste0(as.character(completion_time_FSA), " ", timeZone), printMessage = FALSE)
    FSA_logRecorder("", printMessage = FALSE)
    FSA_logRecorder("", printMessage = FALSE)
