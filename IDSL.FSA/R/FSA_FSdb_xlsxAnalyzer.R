FSA_FSdb_xlsxAnalyzer <- function(spreadsheet) {
  ...
  if (typeof(spreadsheet) == "list") {
    if (ncol(spreadsheet) >= 4) {
      ...
      print("The `FSDB` spreadsheet tab was not produced properly!")
    }
    ...
    if (typeof(spreadsheet) == "character") {
      if (file.exists(spreadsheet)) {
        ...
        print("The `FSDB` spreadsheet tab not found! It should be an Excel file with .xlsx extention!")
      }
    } else {
      print("The `FSDB` spreadsheet tab was not produced properly!")
    }
  ...
  if (checkpoint_parameter) {
    ...
    if (length(x0001) == 0) {
      print("ERROR!!! Problem with FSdb0001!")
      checkpoint_parameter <- FALSE
    } else {
      ...
      if (!dir.exists(input_path_library)) {
        print("ERROR!!! Problem with FSdb0001! Please make sure the full path is provided!")
        checkpoint_parameter <- FALSE
      }
    }
    ##
    ...
    if (length(x0002) == 0) {
      print("ERROR!!! Problem with FSdb0002!")
      checkpoint_parameter <- FALSE
    } else {
    ...
    if (tolower(library_string) == "all") {
      ...
      if (length(file_name_library_msp) > 0) {
        if (is.na(file_name_library_msp[1])) {
          print("ERROR!!! Problem with FSdb0002! No .msp file was detected in the designated folder!")
          checkpoint_parameter <- FALSE
        }
      } else {
        print("ERROR!!! Problem with FSdb0002! No .msp file was detected in the designated folder!")
      ...
      if (length(ID) > 0) {
        print("ERROR!!! Problem with FSdb0002! not detected the following file(s) (case sensitive even for file extensions):")
        for (i in ID) {
          i
        }
        checkpoint_parameter <- FALSE
    ...
    if (length(number_processing_threads) == 0) {
      ...
      if (number_processing_threads >= 1) {
        ...
        print("ERROR!!! Problem with FSdb0005! This parameter should be at least 1 !")
        checkpoint_parameter <- FALSE
      }
    }
    ##
    ...
    if (length(noiseRemovalRatio) == 0) {
      print("ERROR!!! Problem with FSdb0007! This parameter should be a positive number!")
      checkpoint_parameter <- FALSE
    } else {
    ...
    if (!allowedNominalMass) {
      ...
      if (length(massIntegrationWindow) == 0) {
        print("ERROR!!! Problem with FSdb0008! This parameter should be a positive number!")
        checkpoint_parameter <- FALSE
      } else {
