mspPosNegSplitter <- function(path = getwd(), MSPfile = "", number_processing_threads = 1) {
  ##
  ...
  msp <- tryCatch(readLines(mspFileLocation, warn = FALSE),
                  error = function(e){message(paste0("Problem with deconvoluting .msp file --> ", mspFileName))})
  msp <- c("", msp, "")
  ##############################################################################
  ...
  if (length(xIonModeNeg) > 0) {
    ...
    print("'Negative' mode blocks were not detected!")
  }
  ##############################################################################
  ...
  if (length(xIonModePos) > 0) {
    ...
    print("'Positive' mode blocks were not detected!")
  }
  ##############################################################################
  ...
  if (ionModeNegCheck) {
    ...
    tryCatch(write.table(negativeMSPblocks, file = paste0(filename, "_Neg.msp"), quote = FALSE, sep = "\n", row.names = FALSE, col.names = FALSE),
             error = function(e) {message("The saving address is problematic!!!")})
  }
  ##
  if (ionModePosCheck) {
    ##
  ...
  if ((ionModeNegCheck) | (ionModePosCheck)) {
    message(paste0("'", mspFileName, "' was splitted in positive and negative msp blocks and stored in the same folder!"))
  }
  ##
  return()
}