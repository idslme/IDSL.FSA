FSdb_file_generator <- function(PARAM_FSdb, output_path = NULL) {
  ...
  .GlobalEnv$.logFSA <- paste0(address_FSDB, "/logFSA_FSDB.txt")
  ...
  FSA_logRecorder("", printMessage = FALSE)
  FSA_logRecorder("", printMessage = FALSE)
  FSA_logRecorder(paste0(PARAM_FSdb[, 1], "\t", PARAM_FSdb[, 2]),  printMessage = FALSE)
  FSA_logRecorder(paste0(rep("", 100), collapse = "-"))
  ##
  ##############################################################################
  ##
  ...
  FSA_logRecorder(paste0(as.character(completion_time_FSdb), " ", timeZone), printMessage = FALSE)
  FSA_logRecorder("", printMessage = FALSE)
  FSA_logRecorder("", printMessage = FALSE)
  FSA_logRecorder("Completed generating the FSDB workflow!")
  FSA_logRecorder(paste0(rep("", 100), collapse = "="), printMessage = FALSE)
  ##
  ##############################################################################
  ##
  return(libFSdb)
